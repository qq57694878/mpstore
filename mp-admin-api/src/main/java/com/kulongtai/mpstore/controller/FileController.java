...
public class FileController {
    ...
    public boolean isAllow(String contentType ){
        List<String> array = Arrays.asList("image/bmp","image/gif","image/jpeg","image/png","image/tiff","image/webp");
    ...
    public void fileUpload(MultipartHttpServletRequest request,
        ...
        while (itr.hasNext()) {
            ...
            //存储的文件
            File file  =new File(uploadPath+File.pathSeparator+newFileName);
            IOUtils.copy(item.getInputStream(),new FileOutputStream(file));
            JSONObject obj = new JSONObject();
            obj.put("error", 0);
            obj.put("url", getDownloadUrl(File.pathSeparator+newFileName));
